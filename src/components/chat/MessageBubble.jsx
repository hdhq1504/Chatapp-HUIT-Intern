...
function MessageBubble({ message, isFirst, isLast, isGrouped, isNewSession = false, timeSeparator = null }) {
  ...
  const handleFileDownload = useCallback(async (file, fileIndex) => {
      if (!file) return;
        ...
        return (
          ...
          >
            ...
            <div className='absolute top-2 right-2 opacity-0 transition-opacity group-hover:opacity-100'>
              ...
              >
                <Download size={16} className='cursor-pointer flex-shrink-0 text-gray-100 transition-colors hover:text-blue-500' />
              </button>
            </div>
          </div>
        );
    ...
    if (message.type === 'text') {
      return (
        <div
          ...
          `}
        >
          <p className={`text-[15px] ${isLongMessage ? 'leading-relaxed' : 'leading-snug'}`}>
            {message.content}
          </p>
        </div>
      );
        ...
        <div className={`flex flex-col ${isSelf ? 'items-end' : 'items-start'}`}>
          {message.text?.trim() && (
            <div
              className={`
                mb-2 break-words  
                ${isSelf ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900 dark:bg-[#212121] dark:text-white'} 
                ${isLongTextWithFiles ? 'rounded-2xl px-4 py-3' : 'rounded-full px-3 py-2'}
              `}
            >
          ...
          <div className={`flex flex-col gap-2 ${isSelf ? 'items-end' : 'items-start'}`}>
            ...
            {videos.map((file, index) => {
              ...
              return (
                <div
                  key={`video-${index}`}
                  className='group relative max-w-xs overflow-hidden rounded-lg md:max-w-md'
                >
                  <video
                    src={videoUrl}
                    className='h-full w-full object-cover'
            ...
            {otherFiles.map((file, index) => (
              ...
              >
                ...
                <div className='flex-shrink-0'>
                  <Download size={16} className='cursor-pointer flex-shrink-0 text-neutral-600 dark:text-gray-100 transition-colors hover:text-blue-500' />
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <>
      ...
      <div className={`flex ${isSelf ? 'justify-end' : 'justify-start'} ${isGrouped ? 'mt-0.5' : isNewSession ? 'mt-6' : 'mt-3'}`}>
